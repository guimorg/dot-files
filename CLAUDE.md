...
3. **GNU Stow** (via flake.nix) - Dotfiles symlinking
   - Application configs from subdirectories: direnv, dbt, emacs, bash, config, git, tmux, vim, zsh, ohmyposh, kanata, wezterm, claude
...
- **claude/**: `.claude/` (Claude Code configuration - `settings.json`, `plugins/blocklist.json` only; sensitive data excluded)
...
**Other**: tmux, direnv, stow, ledger, hledger, aspell, hunspell, proton-pass-cli, claude-code
...
All stow operations target `$HOME` and manage these directories: `direnv dbt emacs bash config git tmux vim zsh ohmyposh kanata wezterm claude`
...
Claude Code settings are version-controlled via the `claude/` stow package. Only safe configuration files are tracked:
...
The `.gitignore` uses a whitelist approach - everything in `.claude/` is ignored by default, and only the safe config files are explicitly allowed. This prevents accidental commits of sensitive data even as Claude Code evolves.

**Editing config:**
Since files are symlinked via stow, you can edit directly:
```bash
vim ~/.claude/settings.json
cd ~/projects/oss/dot-files && git diff claude/
```
