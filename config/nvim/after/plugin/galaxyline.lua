...
table.insert(gls.right, {
	FileName = {
		provider = function()
			...
			local function pathshorten_with_dummy(path)
				local dummy = "/dummy"
				local path_with_dummy = path .. dummy
				local shortened_path_with_dummy = vim.fn.pathshorten(path_with_dummy)
				local shortened_path = shortened_path_with_dummy:gsub(dummy, "")
				return shortened_path
			end

			if full_path:find(cwd, 1, true) == 1 then
				local root = vim.fn.fnamemodify(cwd, ":h")
				local relative_path = full_path:sub(#root + 2) -- Extract the part of the path after cwd
				return pathshorten_with_dummy(root) .. "/" .. relative_path
			else
				-- File is outside the current working directory
				return vim.fn.pathshorten(full_path) -- Just show the full path
			end
		end,
